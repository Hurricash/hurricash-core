...
contract Hurricash {
    ...
    address payable public relayerAddress = 0x5667F797bb3bede2FDF6fe53BFE5EED6A8247434;

    // Maximum number of participants in a ring
    uint256 constant ringMaxParticipants = 6; // to be adjusted
    ...
    uint256[10] allowedAmounts = [ 1 ether, 2 ether, 4 ether, 8 ether, 50 ether ]; // to be changed on main-net to more suitable amounts based on pulse value
    ...
    function deposit(uint256[2] memory publicKey) public payable
    {
        ...
        // in ETH (PULSE) deposited
        // i.e. 2.1 will give 1, 2.6 will give 6
        // if it's greater than 1, then refund the user
        // otherwise we'll count 0.1 ETH (Pulse) as a donation to our relayer pool (for convenient gasless anonymous withdrawals)
        ...
        returns (uint256)
    {
        ...
        require(allowed, "Only proper ETH values");
